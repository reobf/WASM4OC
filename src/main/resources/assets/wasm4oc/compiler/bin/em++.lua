...
local function emcc(args)
    ...
    while i <= #args do
        if args[i] == "-o" then
            ...
            local a = args[i]
			if not (a:sub(1,1) == "-") and (a:match("%.cpp$")) or a:match("%.c$") then
				inputFile = a
				isCpp = a:match("%.cpp$")
			else
    ...
    if not inputFile then
        io.stderr:write("em++: no input file\n")
        return 1
    end
    ...
    if not inFs then
        io.stderr:write("em++: cannot find filesystem for " .. inputFile .. "\n")
        return 1
    end
    ...
    if not outFs then
        ...
        if not outFs then
            io.stderr:write("em++: cannot find filesystem for output " .. outputFile .. "\n")
            return 1
        end
    end
    ...
    local token = wasm.compileCpp(inFs.address, inRelPath, table.unpack(extraArgs))
...
local args = {...}
if #args == 0 then
    io.write("Usage: em++ <input> [-o output] [args...]\n")
    os.exit(0)
end
