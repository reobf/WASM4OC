...
public class InterpreterMachine implements Machine {
	...
	protected long[] call(MStack stack, Instance instance, List<StackFrame> callStack, int funcId, long[] args,
		...
		if (func != null) {
			...
			try {
				...
				if (results != null) {
				    int ri = 0;
				    for (ValType retType : type.returns()) {
				        if (retType.equals(ValType.V128)) {
				            stack.push(results[ri++], ValType.V128);
				            stack.push(results[ri++], ValType.V128);
				        } else {
				            stack.push(results[ri++], retType);
				        }
				    }
				}
	...
	private boolean CALL_INDIRECT_ASYNC(MStack stack2, Instance instance2, List<StackFrame> callStack2,
		...
		if (useCurrentInstanceInterpreter(instance, refInstance, funcId)) {
			precall( funcId, args, null, false);
			return true;
		} else {
			checkInterruption();
			//TODO: COUNT
			/*var results =*/ ((InterpreterMachine)refInstance.getMachine()).precall_indirect(funcId, args,null,true,tableIdx,funcTableIdx);
			/*if (results != null) {
	...
	private static void REF_NULL(MStack stack) {
		stack.push(REF_NULL_VALUE, ValType.builder().fromId(ValType.ID.RefNull).build());
	}
	...
	protected void CALL_ASYNC(Operands operands) {
		var funcId = (int) operands.get(0);
		var typeId = instance.functionType(funcId);
		var type = instance.type(typeId);
		// given a list of param types, let's pop those params off the stack
		...
		var args = extractArgsForParams(stack, type.params());
		//call(stack, instance, callStack, funcId, args, type, false);
		precall(funcId, args, type, false, null);
	}
	...
	private static void TABLE_GET(MStack stack, Instance instance, Operands operands) {
		...
		stack.push(OpcodeImpl.unboxFromTable(ref, instance, table.elementType()), table.elementType());
	}
	...
	private static void GLOBAL_GET(MStack stack, Instance instance, Operands operands) {
		...
		if (instance.global(idx).getType().equals(ValType.V128)) {
			...
			stack.push(instance.global(idx).getValueLow(),type);
		}
	...
	private static void SELECT_T(MStack stack, Operands operands) {
		...
		if (typeId == ValType.V128.id()) {
			...
			var bType = stack.peekType(); var b = stack.pop();
			var aType = stack.peekType(); var a = stack.pop();
			if (pred == 0) {
			    stack.push(b, bType);
			} else {
			    stack.push(a, aType);
	...
	private static void LOCAL_GET(MStack stack, Operands operands, StackFrame currentStackFrame) {
		...
		if (currentStackFrame.localType(idx).equals(ValType.V128)) {
			...
			stack.push(currentStackFrame.local(i),currentStackFrame.localType(idx));
		}
	}
	...
	private static void I32_ATOMIC_RMW_CMPXCHG(MStack stack, Instance instance, Operands operands) {
		...
		var oldVal = instance.memory((int) operands.get(2)).atomicCmpxchgInt(ptr, expected, replacement);
		stack.push(oldVal);
	}
	...
	private static void I64_ATOMIC_RMW_CMPXCHG(MStack stack, Instance instance, Operands operands) {
		...
		var oldVal = instance.memory((int) operands.get(2)).atomicCmpxchgLong(ptr, expected, replacement);
		stack.push(oldVal);
	}
	...
	private static void I64_ATOMIC_RMW8_CMPXCHG_U(MStack stack, Instance instance, Operands operands) {
		...
		var ptr = readMemPtr(stack, operands);
		var oldVal = instance.memory((int) operands.get(2)).atomicCmpxchgByte(ptr, expected, replacement);
		stack.push(Byte.toUnsignedLong(oldVal));
	}
	...
	private static void I64_ATOMIC_RMW16_U(MStack stack, Instance instance, Operands operands, AtomicOp op) {
		...
		case ADD:
			...
			break;
		case SUB:
			oldVal = instance.memory((int) operands.get(2)).atomicAddShort(ptr, (short) -operand);
			break;
		case AND:
	...
	private static void I64_ATOMIC_RMW16_CMPXCHG_U(MStack stack, Instance instance, Operands operands) {
		...
		var oldVal = instance.memory((int) operands.get(2)).atomicCmpxchgShort(ptr, expected, replacement);
		stack.push(Short.toUnsignedLong(oldVal));
	}
	...
	private static StackFrame RETURN_CALL_INDIRECT(MStack stack, Instance instance, List<StackFrame> callStack,
		...
		if (!refInstance.equals(instance) && !refMachine.equals(instance.getMachine().getClass())) {
			throw new ChicoryException("Indirect tail-call to a different Machine implementation is not supported: "
					+ refMachine.getName());
		}
	...
	private static void TRY_TABLE(StackFrame frame, MStack stack, Instance instance, AnnotatedInstruction instruction,
			int pc) {
		var paramsSize = numberOfParams(instance, instruction);
		var returnsSize = numberOfValuesToReturn(instance, instruction);
	...
	private static boolean functionTypeMatch(FunctionType actual, FunctionType expected, TypeSection ts) {
	...
	protected static void verifyIndirectCall(FunctionType actual, FunctionType expected, TypeSection ts)
	...
	protected static void verifyIndirectCallByTypeIdx(int actualTypeIdx, int expectedTypeIdx, TypeSection ts)
			throws ChicoryException {
		if (actualTypeIdx != expectedTypeIdx && !ValType.heapTypeSubtype(actualTypeIdx, expectedTypeIdx, ts)) {
			throw new ChicoryException("indirect call type mismatch");
		}
	}

	/**
	 * Terminate WASM execution if requested. This is called at the start of each
	 * call and at any potentially backwards branches. Forward branches and other
	 * non-branch instructions are not checked, as the execution will run until it
	 * eventually reaches a termination point.
	 */
	...
	private static void REF_I31(MStack stack) {
		...
		stack.push(Value.encodeI31(val),ValType.I31Ref);
	}
	...
	private static void STRUCT_NEW(MStack stack, Instance instance, Operands operands) {
		...
		stack.push(instance.registerGcRef(struct),ValType.StructRef);
	}
	...
	private static void STRUCT_GET(MStack stack, Instance instance, Operands operands, OpCode opcode) {
		var typeIdx = (int) operands.get(0);
		...
		ValType vt = ft.storageType().packedType() != null ? ValType.I32 : ft.storageType().valType();
		stack.push(val,vt);
	}
	...
	private static void ARRAY_NEW(MStack stack, Instance instance, Operands operands) {
		...
		stack.push(instance.registerGcRef(arr),ValType.ArrayRef);
	}
	...
	private static void ARRAY_NEW_DEFAULT(MStack stack, Instance instance, Operands operands) {
		...
		var elems = new long[len];
		if (at.fieldType().storageType().valType() != null && at.fieldType().storageType().valType().isReference()) {
	...
	private static void ARRAY_GET(MStack stack, Instance instance, Operands operands, OpCode opcode) {
		var typeIdx = (int) operands.get(0);
		...
		ValType vt = at.fieldType().storageType().packedType() != null ? ValType.I32 : at.fieldType().storageType().valType();
	...
	private static void ARRAY_COPY(MStack stack, Instance instance) {
		// operands 0 and 1 are dst/src type indices (used for validation, not needed at
		// runtime)
		var len = (int) stack.pop();
	...
	private static void REF_TEST(MStack stack, Instance instance, Operands operands, OpCode opcode) {
		var heapType = (int) operands.get(0);
		var sourceHeapType = (int) operands.get(1);
		var ref = stack.pop();
		boolean nullable = (opcode == OpCode.REF_TEST_NULL);
		stack.push(instance.heapTypeMatch(ref, nullable, heapType, sourceHeapType) ? Value.TRUE : Value.FALSE);
	}

	private static void CAST_TEST(MStack stack, Instance instance, Operands operands, OpCode opcode) {
		var heapType = (int) operands.get(0);
		var sourceHeapType = (int) operands.get(1);
		var ref = stack.pop();
	...
	private static void BR_ON_CAST(MStack stack, Instance instance, StackFrame frame, AnnotatedInstruction instruction,
			Operands operands) {
	...
	private static void BR_ON_CAST_FAIL(MStack stack, Instance instance, StackFrame frame,
			AnnotatedInstruction instruction, Operands operands) {
		var flags = (int) operands.get(0);
		var ht2 = (int) operands.get(3);
		var sourceHeapType = (int) operands.get(4);
		boolean null2 = (flags & 2) != 0;
	...
	static final class PendingCall implements Serializable{
		final int frame;
		final int funID;
		final int returnSize;
		final boolean popResults;
		final boolean isWasm;
		final long[] args; 
		int tableIdx=-1;int funcTableIdx=-1;
		final public List<ValType> returnType;
		public PendingCall withIndex(int i, int findex2) {
			this.tableIdx=i;
			funcTableIdx=findex2;
		return this;}
		PendingCall(StackFrame frame, FunctionType type, boolean popResults, boolean isWasm, long[] args) {
			this.frame = frame.UID;
			funID = frame.funcId();
			returnSize = sizeOf(type.returns());
	...
	public void precall(
	        ...
	        boolean popResults,int... count) {

	    checkInterruption();
	    var typeId = instance.functionType(funcId);
	    var type = instance.type(typeId);
	    if (callType != null) verifyIndirectCall(type, callType, instance.module().typeSection());
	...
	public void precall_indirect(
	        ...
	        long[] args,
	        FunctionType callType,
	        boolean popResults,int tableIdx,int funcTableIdx,int... count) {
	...
	public long[] postcall() {
	    var completed = completedStack.pop();

	    if (!completed.popResults) return null;
	    if (completed.returnSize == 0) return null;
	    if (stack.size() == 0) return null;

	    var results = new long[completed.returnSize];
	    for (var i = completed.returnSize - 1; i >= 0; i--) {
	        results[i] = stack.pop();
	    }
	    return results;
	}
	...
	protected boolean  asynceval(MStack stack, Instance instance, List<StackFrame> callStack,int... count) throws ChicoryException {
		...
		loop: while (!frame.terminated() && frame.ctrlStackSize() > 0) {
		        ...
		        if (frame.callState == 2) {
		        	
		      // PendingCall call =  (PendingCall) pendingStack.toArray()[pendingStack.size()- depth[0]-1];
		        var call=frame.pendingcall;	
		        	int index=call.tableIdx;
		        	if(index==-1)throw new AssertionError();
		        
		    		var tableIdx = index;
		    		var table = instance.table(tableIdx);
		    		Instance refInstance = requireNonNullElse(table.instance(call.funcTableIdx), instance);
		    		InterpreterMachine ip= (InterpreterMachine) refInstance.getMachine();
		         	...
		         	depth[0]++;
		            boolean childDone = ip.docall( count);
		            depth[0]--;
		            if (!childDone) return false; // budget 还是不够
		            postcall();
		            frame.callState = 0;
			...
			case CALL_INDIRECT:
				if(CALL_INDIRECT_ASYNC(stack, instance, callStack, operands)) {
					frame.callState=1;
				}else{frame.callState=2;}
				break;
			case DROP:
				DROP(stack, operands);
				break;
			...
			case TABLE_GROW:
				...
				break;
			case REF_FUNC:
				stack.push(operands.get(0), ValType.FuncRef);
				break;
			case REF_NULL:
				REF_NULL(stack);
				break;
	...
	private void CALL_REF_ASYNC() {
		...
		// and pass as args to the function call
		var args = extractArgsForParams(stack, type.params());
		precall(funcId, args, type, false);
