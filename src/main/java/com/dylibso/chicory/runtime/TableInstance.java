...
public class TableInstance implements Serializable{
    ...
    transient public Instance[] instances;
    private int[] refs;
    private void readObject(ObjectInputStream in)
        ...
        if (in instanceof InstanceAwareObjectInputStream) {
            Instance inst = ((InstanceAwareObjectInputStream) in).getIns();
            instances = new Instance[(int)table.limits().min()];
            Arrays.fill(instances, inst);
        }
    }
